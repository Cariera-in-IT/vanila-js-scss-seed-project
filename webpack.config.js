...
module.exports = (webpack_env) => {
    ...
    if (!process.env.NETLIFY) {
        ...
        repositoryName =
            gitRemote &&
            gitRemote.match(gitSearch) &&
            gitRemote.match(gitSearch)[5];
        const gitUserName =
            gitRemote &&
            gitRemote.match(gitSearch) &&
            gitRemote.match(gitSearch)[4];
        console.log(
            "Your github pages url should be:",
            `https://${gitUserName}.github.io/${repositoryName}/`
        );
    }
    const baseHref =
        //If webpack server or no repository name provided
        process.env.WEBPACK_SERVE ||
        !repositoryName ||
        process.env.NETLIFY || // If builds on the netlify server
        process.env.LOCAL_NETLIFY || //Env variable for local netlify build
        webpack_env.netlify // If specified to build with netlify config
            ? "/"
            : `/${repositoryName}/`;
    ...
    return {
        ...
        context: absoluteSrc,
        devtool: "eval",
        entry: [`./js/main.js`, "./styles/main.scss", "./typescript/main.ts"],
        optimization: {
            minimize: false,
        },
        output: {
            ...
            rules: [
                ...
                },
                {
                    ...
                    options: {
                        multiple: [
                            {
                                search: 'src="/scripts.bundle.js"',
                                replace: `src="${baseHref}scripts.bundle.js"`,
                            },
                            {
                                search: /<a([^>]+)href="\/(.*?)"/gm,
                                replace: `<a$1href="${baseHref}$2"`,
                            },
                            {
                                search: /<link([^>]+)href="\/(.*?.css)"/gm,
                                replace: `<link$1href="${baseHref}$2"`,
                            },
                        ],
                    },
                },
